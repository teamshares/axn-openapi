...
module Axn
  module OpenAPI
    ...
    # `Axn::Reflection::UnserializableValue`, which the Dispatcher maps to a generic 500. Default
    # `true`, unlike axn-mcp's `false`: an LLM tool result can live with an ugly-but-honest string,
    # but a published HTTP contract shipping `#<…>` is a bug. Applies ONLY to outbound `exposes`
    # serialization, not to inbound argument handling. (Values with no *honest* JSON form — cycles,
    # non-finite Floats, non-UTF-8-encodable bytes, colliding property names — raise regardless of
    # this flag; it governs only the extra "was this rendering author-declared?" check.)
    #
